...
export default function AppLogo({ size = 'md', subtitle, className, market }) {
  const sizes = {
    ...
    <div className={cn('flex items-center gap-3', className)}>
      ...
      <div className="min-w-0">
        <div className={cn('font-semibold leading-tight tracking-tight text-foreground', s.title)}>
          {market ? (
            <>
              ASSA <span className="text-primary font-medium">Fish Market</span>
            </>
          ) : (
            'ASSA'
          )}
        ...
        {subtitle && (
          <div className={cn('text-muted-foreground truncate', s.sub)}>{subtitle}</div>
        )}
      </div>
    </div>
  )
